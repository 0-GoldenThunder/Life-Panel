...
export const GrowthMiniChart: React.FC = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    const containerX = (pt.x / width) * rect.width;
    const containerY = (pt.y / height) * rect.height;
  ...
  return (
    <div className="flex flex-col p-6 rounded-2xl bg-[#0A0A0A]/50 border border-[#222] backdrop-blur-xl h-full group hover:border-luxury-gold/30 hover:shadow-[0_0_20px_rgba(212,175,55,0.05)] transition-all duration-500 relative overflow-visible">
      <div className="flex items-center justify-between mb-4 z-10">
        <a href="/growth" className="flex items-center gap-2 cursor-pointer select-none">
          <Icon icon={TrendingUp} className="w-4 h-4 text-platinum/40 group-hover:text-platinum transition-colors" />
        ...
        <div className="flex rounded-lg bg-[#141414] p-0.5 border border-[#222]">
          <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setPeriod('month'); }}
          ...
          <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setPeriod('year'); }}
            className={`text-[9px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded transition-all duration-300 ${
              period === 'year' 
                ? 'bg-luxury-gold/15 text-luxury-gold font-bold border border-luxury-gold/30' 
                : 'text-platinum/40 hover:text-platinum border border-transparent'
      ...
      <div className="flex-1 w-full h-full min-h-[80px] flex items-center justify-center relative select-none">
        ...
        >
          <defs>
            ...
            <filter id="gmMiniGlow" x="-10%" y="-10%" width="120%" height="120%">
              <feGaussianBlur stdDeviation="3" result="blur" />
              <feComposite in="SourceGraphic" in2="blur" operator="over" />
            </filter>
          </defs>

          <line x1="0" y1={height / 2} x2={width} y2={height / 2} stroke="#ffffff" strokeOpacity="0.03" strokeDasharray="3 3" />
          <line x1={width / 2} y1="0" x2={width / 2} y2={height} stroke="#ffffff" strokeOpacity="0.03" strokeDasharray="3 3" />

          <path d={areaPath} fill={isGain ? 'url(#gmGainGrad)' : 'url(#gmLossGrad)'} className="transition-all duration-500 ease-in-out" />
          <path d={linePath} fill="none" stroke={isGain ? '#C5A059' : '#8A3324'} strokeWidth="2" strokeLinecap="round" filter="url(#gmMiniGlow)" className="transition-all duration-500 ease-in-out" />

          {hoveredIndex !== null && points[hoveredIndex] && (
            <>
              <line x1={points[hoveredIndex].x} y1="0" x2={points[hoveredIndex].x} y2={height} stroke={isGain ? '#C5A059' : '#8A3324'} strokeOpacity="0.2" strokeWidth="1" strokeDasharray="2 2" />
              <circle cx={points[hoveredIndex].x} cy={points[hoveredIndex].y} r="6" fill={isGain ? '#C5A059' : '#8A3324'} fillOpacity="0.2" />
              <circle cx={points[hoveredIndex].x} cy={points[hoveredIndex].y} r="3" fill="#ffffff" stroke={isGain ? '#C5A059' : '#8A3324'} strokeWidth="1.5" />
            </>
          )}
        </svg>
        ...
        <AnimatePresence>
          {hoveredIndex !== null && tooltipPos && aggregated[hoveredIndex] && (
            <motion.div
              ...
              style={{ position: 'absolute', left: `${tooltipPos.x - 60}px`, top: `${tooltipPos.y - 90}px`, transform: 'translateX(-50%)' }}
            ...
            >
              ...
              <span className="text-[9px] font-semibold text-platinum/50 uppercase tracking-widest block mb-0.5">{aggregated[hoveredIndex].label}</span>
              <div className="text-[10px] text-platinum/80 mb-0.5">
                Delta:{' '}
                <span className={`font-sans font-semibold tabular-nums ${aggregated[hoveredIndex].cashflow >= 0 ? 'text-luxury-gold' : 'text-soft-crimson'}`}>
                  {aggregated[hoveredIndex].cashflow >= 0 ? '+' : ''}
                  {aggregated[hoveredIndex].cashflow.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                </span>
              </div>
              <div className="text-[11px] font-sans font-semibold tabular-nums text-platinum truncate"><span className="font-serif italic mr-1">{currency}</span><span className="font-sans tabular-nums">{trendValues[hoveredIndex].toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
