...
export const FinanceMiniChart: React.FC = () => {
  ...
  // 4. Normalize coordinates for a 300x100 SVG viewport
  const width = 300;
  const height = 90;
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    // Get the exact bounding box of the SVG element to ensure perfect hover alignment
    const rect = e.currentTarget.getBoundingClientRect();
    const mouseX = e.clientX - rect.left;
    
    // Convert relative container width to SVG coordinate system (300px scale)
    ...
    // Position tooltip near the point in container coordinate space
  ...
  return (
    <div 
      className="flex flex-col p-6 rounded-2xl bg-[#0A0A0A]/50 border border-[#222] backdrop-blur-xl h-full group hover:border-luxury-gold/30 hover:shadow-[0_0_20px_rgba(212,175,55,0.05)] transition-all duration-500 relative overflow-visible"
    >
      {/* Header with Title and Toggle Tabs */}
      <div className="flex items-center justify-between mb-4 z-10">
        <a href="/growth" className="flex items-center gap-2 cursor-pointer select-none">
          <Icon icon={LineChart} className="w-4 h-4 text-platinum/40 group-hover:text-platinum transition-colors" />
      ...
      <div className="flex-1 w-full h-full min-h-[80px] flex items-center justify-center relative select-none">
        <svg 
          ...
          preserveAspectRatio="none" // Stretch the mini-chart completely from left to right to maximize horizontal space
          ...
          {hoveredIndex !== null && tooltipPos && aggregated[hoveredIndex] && (
            ...
            >
              ...
              <div className="text-[10px] text-platinum/80 mb-0.5">
                Delta:{' '}
                <span className={aggregated[hoveredIndex].cashflow >= 0 ? 'text-luxury-gold' : 'text-soft-crimson'}>
              ...
              <div className="text-[11px] font-serif font-bold text-platinum truncate"><span className="font-serif italic mr-1">{currency}</span><span className="font-sans tabular-nums">{trendValues[hoveredIndex].toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
